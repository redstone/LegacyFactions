...
public class CmdFactionsMoneyTransferPf extends FCommand {
    ...
    public void perform() {
        ...
        if (success && Conf.logMoneyTransactions) {
            Factions.get().log(ChatColor.stripColor(Factions.get().getTextUtil().parse(TL.COMMAND_MONEYTRANSFERPF_TRANSFER.toString(), fme.getName(), VaultEngine.moneyString(amount), from.describeTo(null), to.describeTo(null))));
        }
    }

    @Override
    public String getUsageTranslation() {
        return TL.COMMAND_MONEYTRANSFERPF_DESCRIPTION.toString();
    }
}
